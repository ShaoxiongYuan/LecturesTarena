...
1,都会在哪里写 URL
    ...
    1，模板中【html】
       1,<a href='url'>超链接</a>  点击后 地址栏跳转至url
       2,<form action='url' method='post'> form表单数据会提交至action的url
       
	2, 视图函数中(view)
    	302 - 跳转 return HttpResponseRedirect('url')
    

2, URL的编写规则

	1，绝对路径地址  http://127.0.0.1:8000/page/1

	2, 相对路径地址
    
    	1， '/page/1'  '/'开头的相对地址
        	浏览器解读： 当前地址栏中的 协议+ip+端口 + 相对地址，生成最终地址；当前所在位置 http://127.0.0.1:8000/page/3, 最终地址为
http://127.0.0.1:8000/page/1  [推荐]
        
        2,  'page/1'  不带'/'的相对地址
            浏览器解读： 根据当前地址栏中的url 最后一个/ 以前的内容 加上您的 相对地址； http://127.0.0.1:8000/topic/detail,最终地址为 http://127.0.0.1:8000/topic/page/1
...
- path() 的`name`关键字参数
    ...
    - 在模板中 ->通过url标签实现地址的反向解析
        ...
        {% url '别名' '参数值1' '参数值2' %}
        ex:
        {% url 'pagen' '400' %}   path('page/<int:p>', ..., name='pagen')
