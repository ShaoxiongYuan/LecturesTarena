...
sidebar_guoxiaonao ->欢迎 郭小闹
sidebar_wangweichao ->欢迎 王伟超
...
- 缓存api
  ...
  ```PYTHON
  settings.py CACHES = {'default':{}, 'user':{}}
...
​               更快介质 - 不发请求 -  第一次给某url发请求，拿到响应后，浏览器把响应存储到了浏览器的 存储空间中
...
#多个中间件调用顺序
    #从上至下 （process_request  process_view）
    视图函数
    #从下至上 
...
- 练习
    - 用中间件实现强制某个IP地址只能向/test_mw 发送 5 次GET请求
    - 提示:
