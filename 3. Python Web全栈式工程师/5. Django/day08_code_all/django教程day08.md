...
- 在setting.py 中设置MEDIA相关配置；Django把用户上传的文件，统称为media资源
  ...
  ```python
  # file : settings.py
  MEDIA_URL = '/media/' #什么样的请求 为加载media资源请求 http://127.0.0.1:8000/media/a.jpg  #类似 STATIC_URL 
...
- 上传文件的视图处理函数  方案2  借助orm
  ...
  class Content(models.Model):
      ...
      # MEDIA_ROOT + / + upload_to 文件夹下
      # ImageField -> PIL库 完全针对图片存储
...
- 创建用户
    - 创建普通用户create_user
        ...
        user = User.objects.create_user(username='用户名', password='密码明文', email='邮箱',...)
        
        ```
    - 创建超级用户create_superuser
        ```python
        from django.contrib.auth.models import User
        user = User.objects.create_superuser(username='用户名', password='密码明文', email='邮箱',...)
        ```
- 删除用户
    ```python
    from django.contrib.auth.models import User
...
#settings.py添加配置 - 告知django 未来用我这张用户表，弃用原来的auth_user
...
- 修改uWSGI配置 
    ...
    ```shell
    进入到 项目同名文件夹下
    $ sudo uwsgi --stop uwsgi.pid    [ps aux|grep 'uwsgi']
    $ sudo uwsgi --ini uwsgi.ini
    ```
...
```python
注意事项：
    1，记得改完 配置后 重启服务  【uWSGI nginx】
    2, 如果访问127.0.0.1:80  报 502； uWSGI没启动，启动即可
    3，如果访问127.0.0.1:80 阻塞  ， uWSGI 还是http模式， 切换模式，重启即可
```
